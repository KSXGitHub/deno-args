...
export const setup = pass(BLANK)
  ...
  .to(FlaggedCommand, EarlyExitFlag("early-exit", {
    exit() {
      throw "Not meant to be called!";
    },
  }))
  .to(FlaggedCommand, Flag("foo", {
    alias: ["f"],
    describe: "Boolean flag of foo",
  }))
  .to(FlaggedCommand, Flag("bar"))
  .to(FlaggedCommand, CountFlag("count", {
    alias: ["c"],
    describe: "Counting",
  }))
  .to(FlaggedCommand, Option("number", {
    alias: ["N"],
    type: FiniteNumber,
    describe: "An integer or a floating-point number",
  }))
  .to(FlaggedCommand, Option("integer", {
    type: Integer,
    describe: "An arbitrary large integer",
  }))
  .to(FlaggedCommand, Option("text", {
    type: Text,
  }))
  .to(FlaggedCommand, PartialOption("partial-integer", {
    type: Integer,
    describe: "An optional integer",
    default: BigInt("123"),
  }))
  .to(FlaggedCommand, Option("choice", {
  ...
  .to(SubCommand, "sub0" as const, pass(BLANK)
    ...
    .get())
  .to(SubCommand, "sub1" as const, pass(BLANK)
    .to(Describe, "Sub command with one flag")
    .to(FlaggedCommand, Flag("test", {
      describe: "Test flag for sub1",
    }))
    .get())
  .to(SubCommand, "sub2" as const, pass(BLANK)
